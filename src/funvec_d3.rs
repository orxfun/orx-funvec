pub(crate) type Ind = (usize, usize, usize);

/// Trait to provide abstraction over 3-dimensional vectors allowing access using indices.
///
...
pub trait FunVecD3<T> {
    ...
    /// Returns the value at the given indices (`i`, `j`, `k`) or `None` if the position is empty.
    ///
    ...
    ///
    /// `indices` can be any `Iterator` yielding `(usize, usize, usize)` indices.
    ///
...
pub trait FunVecRefD3<T: ?Sized> {
    ...
    /// Returns a reference to the element at the given indices (`i`, `j`, `k`) or `None` if the position is empty.
    ///
    /// This allows to access elements of all funvec implementations in a unified way. Thanks to monomorphization, this abstraction does not have a performance penalty.
    ///
    /// Note that funvec's are different than, generalization of, traditional since the elements are not necessarily contagious or dense.
